		...
		if($renderPager)
		{
			if($pager->haveToPaginate())
			{
				...
				if($page != 1)
				{
					if($page>2){
						$pagerHtml.= self::link_to_remote(
							image_tag(sfDatagrid::getConfig('images_dir') . 'pager-arrow-first.gif', array('alt'=>"", 'align'=>"absmiddle")),
							array(
								'url' => $moduleAction . '?' . $this->P_PAGE . '=' . $pager->getFirstPage() . '&' . $suffixWithSorting,
								'update' => $datagridName,
								'script' => true,
                                                                'loading' => 'dg_hide_show(\'' . $datagridName . '\')',
                                                                'complete'=> '$(\'div#loader-' .$datagridName  . '\').hide();  '
								));
					}
					$pagerHtml.= self::link_to_remote(
							image_tag(sfDatagrid::getConfig('images_dir') . 'pager-arrow-left.gif', array('alt'=>"", 'align'=>"absmiddle")),
							array(
				...
				foreach($pager->getLinks() as $item)
				{
					if($item == $page)
					{
						$pagerHtml.= self::link_to_remote($item,
								array(
				...
				if($page != $pager->getLastPage())
				{
					...
					$pagerHtml.= self::link_to_remote(
                                                        image_tag(sfDatagrid::getConfig('images_dir') . 'pager-arrow-right.gif', array('alt'=>"", 'align'=>"absmiddle")),							
							array(
					...
					if($page+1!=$pager->getLastPage()){
						$pagerHtml.= self::link_to_remote(
							image_tag(sfDatagrid::getConfig('images_dir') . 'pager-arrow-last.gif', array('alt'=>"", 'align'=>"absmiddle")),
							array(
	...
	{ 
		...
		if(is_array($rowValues))
		{
			...
			foreach($rowValues as $value)
			{
				...
				{
                                        $image_dir = sfDatagrid::getConfig('images_dir_orm');
					$type = $filtersTypes[$columnName];
					if($type == "BOOLEAN"){

						if($value)
							$value = image_tag($image_dir.'/tick.png',array('align'=>'center'));
						else
							$value = image_tag($image_dir.'/delete.png',array('align'=>'center'));
					}
				}
